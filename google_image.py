"""
https://senmyou.xyz参考
Google画像で引数のキーワードを検索してサムネイル画像100枚を取得する。
取得画像はカレントディレクトリ下に作成した「キーワード_imagesフォルダ」に保存
画像ファイル名は連番
画像フォルダ名はキーワード＋日付時間

なんだかんだで上限数の7割くらいしか取得できない
"""
# 自分で追加
import sys
from selenium.webdriver.common.by import By #selenium4から必要
import chromedriver_binary #pipでchromedriveｒ-binaryをインストールする際に必要
# ここまで
import requests
...
QUERY = sys.argv[1]  # 検索ワードはコマンドライン引数から取得
LIMIT_DL_NUM = 100  # ダウンロード数の上限
SAVE_DIR = f'images_{QUERY}_{dt_date_str}'  # 出力フォルダへのパス（フォルダがない場合は自動生成する）
FILE_NAME = f''  # ファイル名（ファイル名の後ろに０からの連番と拡張子が付く）
...
driver = webdriver.Chrome() #chromedriverをpipでインストールした際の書き方
...
tmb_elems = driver.find_elements(by=By.CSS_SELECTOR, value='#islmp img')
...
imgframe_elem = driver.find_element(by=By.ID, value='islsp')
...
for tmb_elem, tmb_alt in zip(tmb_elems, tmb_alts):
    ...
    alt = tmb_alt.replace("'", "\\'")
    try:
        # img_elem = imgframe_elem.find_element_by_css_selector(f'img[alt=\'{alt}\']')         #Selenium3
        img_elem = imgframe_elem.find_element(by=By.CSS_SELECTOR, value=f'img[alt=\'{alt}\']') #Selenium4
